...
void make_op::init_GUI(void) {
	...
	mMake_GUI.ptrMat_all_dialog->setWindowTitle(MAT_ALL_STR + "_list");
	
	//mat_mat模块GUI部分
	mMake_GUI.ptrMat_mat_button = new BUTTON(MAT_MAT_STR);
	...
	mMake_GUI.mMat_mat_dialog_0->setWindowTitle(MAT_MAT_STR + "_list");
	mMake_GUI.mMat_mat_dialog_1->setWindowTitle(MAT_MAT_STR + "_list");
...
void make_op::init_button(void) {
	...
	//初始化 Gray 模块按钮,连接Gray创建模块mat->mat
	...
	//初始化 Operator_big 模块按钮,连接Operator创建模块mat->mat
	mMake_button.Operator_big_0 = new BUTTON(">");
	mMake_button.Operator_big_1 = new BUTTON(">");
	connect(mMake_button.Operator_big_0, &QToolButton::clicked, []() {
		if (ptrMake_op == NULL) {
			ptrMake_op = new Operator_big;
			ptrCreate_flag = ModelName::i32Mat_mat;
		}
	});
	mMake_GUI.ptrMat_mat_layout_0->addWidget(mMake_button.Operator_big_0);

	connect(mMake_button.Operator_big_1, &QToolButton::clicked, []() {
		if (ptrMake_op == NULL) {
			ptrMake_op = new Operator_big;
			ptrCreate_flag = ModelName::i32Mat_mat;
		}
	});
	mMake_GUI.ptrMat_mat_layout_1->addWidget(mMake_button.Operator_big_1);
}
